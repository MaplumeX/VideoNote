"""LLM note generation stage (phase=notegen) — async rewrite of app/services/note_gen.py.

Semantics (prompts, retry, continuation, chunking, merging) are migrated
character-for-character from the legacy sync implementation; the call layer is
rewritten on the async OpenAI client with ``asyncio.sleep`` backoff so that
cancellation aborts in-flight requests immediately.
"""

from __future__ import annotations

import asyncio
import logging
from typing import ClassVar

from app.pipeline.errors import ErrorCode, PipelineError
from app.pipeline.stages.base import (
    ProviderEndpoint,
    StageContext,
    StageResult,
)
from app.pipeline.state import ArtifactKind, PipelinePhase
from app.services.markdown import normalize_note_markdown

logger = logging.getLogger(__name__)

MAX_TRANSCRIPT_CHARS = 60000

_PROMPTS_WITH_TIMESTAMPS: dict[str, dict[str, str]] = {
    "en": {
        "system": (
            "You are a professional note-taking assistant. "
            "Given a video transcript with timestamps, "
            "generate well-structured Markdown notes.\n\n"
            "Rules:\n"
            "1. Use proper heading hierarchy (##, ###) to organize topics\n"
            "2. Include bullet points for key information\n"
            "3. Preserve timestamps from the transcript as clickable links "
            "in the format [HH:MM:SS](#t=SECONDS)\n"
            "4. Summarize the main points concisely - "
            "don't just repeat the transcript\n"
            "5. If the transcript has clear sections, "
            "create a summary for each section\n"
            "6. Add a brief overall summary at the top\n"
            "7. Write notes in the same language as the transcript "
            "(Chinese for Chinese, English for English)\n"
            "8. Return only the Markdown document. Do not wrap it in a code block.\n"
        ),
        "user": "Please generate structured Markdown notes from the following video transcript.",
    },
    "zh-CN": {
        "system": (
            "你是一位专业的笔记助手。"
            "给定带有时间戳的视频转录文本，"
            "生成结构化的 Markdown 笔记。\n\n"
            "规则：\n"
            "1. 使用合适的标题层级（##、###）组织主题\n"
            "2. 用要点列出关键信息\n"
            "3. 保留转录文本中的时间戳，"
            "以可点击链接格式 [HH:MM:SS](#t=SECONDS) 呈现\n"
            "4. 简明扼要地总结要点 - 不要照搬转录文本\n"
            "5. 如果转录文本有明显段落，为每段生成摘要\n"
            "6. 在顶部添加简要总览\n"
            "7. 用中文撰写笔记\n"
            "8. 只返回 Markdown 文档本身，不要包裹在代码块中。\n"
        ),
        "user": "请根据以下视频转录文本生成结构化的 Markdown 笔记。",
    },
}

_PROMPTS_WITHOUT_TIMESTAMPS: dict[str, dict[str, str]] = {
    "en": {
        "system": (
            "You are a professional note-taking assistant. "
            "Given a video transcript, "
            "generate well-structured Markdown notes.\n\n"
            "Rules:\n"
            "1. Use proper heading hierarchy (##, ###) to organize topics\n"
            "2. Include bullet points for key information\n"
            "3. The transcript does not contain timestamps, "
            "so do not add any timestamps\n"
            "4. Summarize the main points concisely - "
            "don't just repeat the transcript\n"
            "5. If the transcript has clear sections, "
            "create a summary for each section\n"
            "6. Add a brief overall summary at the top\n"
            "7. Write notes in the same language as the transcript "
            "(Chinese for Chinese, English for English)\n"
            "8. Return only the Markdown document. Do not wrap it in a code block.\n"
        ),
        "user": "Please generate structured Markdown notes from the following video transcript.",
    },
    "zh-CN": {
        "system": (
            "你是一位专业的笔记助手。"
            "给定视频转录文本，"
            "生成结构化的 Markdown 笔记。\n\n"
            "规则：\n"
            "1. 使用合适的标题层级（##、###）组织主题\n"
            "2. 用要点列出关键信息\n"
            "3. 转录文本不包含时间戳，因此不要添加任何时间戳\n"
            "4. 简明扼要地总结要点 - 不要照搬转录文本\n"
            "5. 如果转录文本有明显段落，为每段生成摘要\n"
            "6. 在顶部添加简要总览\n"
            "7. 用中文撰写笔记\n"
            "8. 只返回 Markdown 文档本身，不要包裹在代码块中。\n"
        ),
        "user": "请根据以下视频转录文本生成结构化的 Markdown 笔记。",
    },
}

def _is_retryable(exc: Exception) -> bool:
    """Return True if the LLM exception is worth retrying."""
    from openai import (
        APIConnectionError,
        APIStatusError,
        APITimeoutError,
        RateLimitError,
    )

    if isinstance(exc, (RateLimitError, APITimeoutError, APIConnectionError)):
        return True
    if isinstance(exc, APIStatusError):
        return exc.status_code >= 500
    return False

class LLMClient:
    """Thin async OpenAI wrapper with retry and continuation.

    Injectable and replaceable for tests (``FakeLLMClient`` implements the
    same surface). Constructing the client is cheap; the underlying async
    ``OpenAI`` manages its own HTTP transport.
    """

    def __init__(self, endpoint: ProviderEndpoint) -> None:
        from openai import AsyncOpenAI

        self._endpoint = endpoint
        self._client = AsyncOpenAI(api_key=endpoint.api_key, base_url=endpoint.api_base)

    async def _create_with_retry(
        self,
        messages: list[dict[str, str]],
        *,
        temperature: float,
        max_tokens: int,
    ) -> object:
        """Call chat.completions.create with exponential-backoff retry."""
        max_attempts = 3
        for attempt in range(max_attempts):
            try:
                return await self._client.chat.completions.create(
                    model=self._endpoint.model,
                    messages=messages,
                    temperature=temperature,
                    max_tokens=max_tokens,
                )
            except asyncio.CancelledError:
                raise
            except Exception as e:
                if attempt + 1 >= max_attempts or not _is_retryable(e):
                    raise
                wait = 2 ** (attempt + 1)  # 2s, 4s
                logger.warning(
                    f"LLM call attempt {attempt + 1} failed, retrying in {wait}s: {e}"
                )
                await asyncio.sleep(wait)
        return None  # unreachable

    async def complete(
        self,
        messages: list[dict[str, str]],
        *,
        temperature: float = 0.3,
        max_tokens: int = 8192,
    ) -> str:
        """Call the LLM with exponential-backoff retry for transient failures.

        If the response is truncated (finish_reason == "length"), up to 2
        continuation requests are made to complete the output.
        """
        response = await self._create_with_retry(
            messages, temperature=temperature, max_tokens=max_tokens
        )
        content = response.choices[0].message.content or ""
        finish_reason = response.choices[0].finish_reason

        continuations = 0
        while finish_reason == "length" and continuations < 2:
            continuations += 1
            cont_messages = [
                *messages,
                {"role": "assistant", "content": content},
                {
                    "role": "user",
                    "content": (
                        "Continue exactly where you left off. "
                        "Do not repeat, do not wrap in a code block."
                    ),
                },
            ]
            response = await self._create_with_retry(
                cont_messages, temperature=temperature, max_tokens=max_tokens
            )
            content += response.choices[0].message.content or ""
            finish_reason = response.choices[0].finish_reason

        if finish_reason == "length":
            logger.warning(f"LLM output still truncated after {continuations} continuations")

        return content

def split_transcript(transcript: str, max_chars: int = MAX_TRANSCRIPT_CHARS) -> list[str]:
    """Split transcript at line boundaries, each chunk <= max_chars.

    If a single line exceeds max_chars it is kept as-is (cannot split further
    without breaking a timestamp entry).
    """
    if len(transcript) <= max_chars:
        return [transcript]
    lines = transcript.split("\n")
    chunks: list[str] = []
    current: list[str] = []
    current_len = 0
    for line in lines:
        line_len = len(line) + 1  # +1 for the \n we'll rejoin with
        if current_len + line_len > max_chars and current:
            chunks.append("\n".join(current))
            current = []
            current_len = 0
        current.append(line)
        current_len += line_len
    if current:
        chunks.append("\n".join(current))
    return chunks

def _build_generation_messages(
    transcript: str,
    video_title: str | None,
    language: str,
    has_timestamps: bool,
) -> list[dict[str, str]]:
    """Build the [system, user] message pair for one transcript chunk."""
    prompts_map = _PROMPTS_WITH_TIMESTAMPS if has_timestamps else _PROMPTS_WITHOUT_TIMESTAMPS
    prompts = prompts_map.get(language, prompts_map["en"])

    if video_title:
        title_context = (
            f"\n\nVideo title: {video_title}"
            if language == "en"
            else f"\n\n视频标题：{video_title}"
        )
    else:
        title_context = ""

    transcript_label = "转录文本" if language == "zh-CN" else "Transcript"
    user_content = f"{prompts['user']}{title_context}\n\n{transcript_label}:\n{transcript}"

    return [
        {"role": "system", "content": prompts["system"]},
        {"role": "user", "content": user_content},
    ]

def _build_merge_messages(
    sub_notes: list[str],
    video_title: str | None,
    language: str,
) -> list[dict[str, str]]:
    """Build the [system, user] message pair that merges sub-notes."""
    if language == "zh-CN":
        system = (
            "你是一位笔记整合助手。下面是同一视频的多段笔记草稿，按时间顺序拼接。"
            "请整合为一篇连贯的 Markdown 笔记：\n"
            "1. 统一标题层级（## 用于大主题，### 用于子主题）\n"
            "2. 去除重复内容\n"
            "3. 在顶部添加简要总览\n"
            "4. 保留所有时间戳链接\n"
            "5. 只返回 Markdown 文档本身，不要包裹在代码块中。\n"
        )
        user_prefix = "请整合以下多段笔记草稿："
    else:
        system = (
            "You are a note integration assistant. Below are multiple note drafts "
            "from the same video, concatenated in order. "
            "Merge them into a single coherent Markdown note:\n"
            "1. Unify heading hierarchy (## for major topics, ### for subtopics)\n"
            "2. Remove duplicates\n"
            "3. Add a brief overview at the top\n"
            "4. Preserve all timestamp links\n"
            "5. Return only the Markdown document. Do not wrap it in a code block.\n"
        )
        user_prefix = "Please integrate the following note drafts:"

    combined = "\n\n---\n\n".join(sub_notes)
    user_content = f"{user_prefix}\n\n{combined}"

    return [
        {"role": "system", "content": system},
        {"role": "user", "content": user_content},
    ]

class NoteGenStage:
    """LLM note generation phase (notegen).

    Reads the ``transcript`` artifact, generates Markdown notes via the LLM
    (single chunk, or per-chunk sub-notes merged with a final call), and
    returns the normalized notes through ``StageResult.extra["notes"]``.
    """

    phase: ClassVar[PipelinePhase] = PipelinePhase.notegen

    def __init__(self, client: LLMClient | None = None) -> None:
        """``client`` allows tests to inject a FakeLLMClient (zero network)."""
        self._client = client

    async def run(self, ctx: StageContext, *, resume: bool) -> StageResult:
        # resume=True: no intermediate state is persisted (sub-notes stay
        # in memory), so the whole phase re-runs — LLM calls are side-effect
        # free, making this idempotent and safe.
        try:
            return await self._run(ctx)
        except PipelineError:
            raise
        except asyncio.CancelledError:
            raise
        except Exception as e:
            raise PipelineError(
                ErrorCode.NOTE_GENERATION_FAILED,
                detail=str(e),
                cause=e,
            ) from e

    async def _run(self, ctx: StageContext) -> StageResult:
        endpoint = ctx.provider.llm
        if not endpoint.is_complete():
            raise PipelineError(ErrorCode.PROVIDER_NOT_CONFIGURED)

        transcript_obj = await ctx.artifacts.get(ArtifactKind.transcript)
        transcript = transcript_obj if isinstance(transcript_obj, str) else None
        if not transcript:
            # Defensive: the transition table guarantees transcribe/subtitle
            # ran before notegen, so the artifact must exist.
            raise PipelineError(
                ErrorCode.PROCESSING_FAILED,
                detail="transcript artifact missing",
            )

        client = self._client or LLMClient(endpoint)

        video_title = await self._video_title(ctx)
        has_timestamps = "#t=" in transcript
        chunks = split_transcript(transcript)
        total = len(chunks)

        if total == 1:
            await ctx.progress.publish(
                PipelinePhase.notegen, 0.0, "Generating notes..."
            )
            notes = await client.complete(
                _build_generation_messages(chunks[0], video_title, ctx.language, has_timestamps)
            )
            await ctx.progress.publish(
                PipelinePhase.notegen, 1.0, "Notes generated"
            )
        else:
            sub_notes: list[str] = []
            for i, chunk in enumerate(chunks):
                # Cancellation points: each chunk is a separate await; an
                # asyncio.Task.cancel() aborts the in-flight request and no
                # further chunks are started.
                await ctx.progress.publish(
                    PipelinePhase.notegen,
                    (i + 1) / total * 0.9,
                    f"Generating notes {i + 1}/{total}",
                )
                sub_notes.append(
                    await client.complete(
                        _build_generation_messages(
                            chunk, video_title, ctx.language, has_timestamps
                        )
                    )
                )
            await ctx.progress.publish(
                PipelinePhase.notegen, 0.9, "Merging notes..."
            )
            notes = await client.complete(
                _build_merge_messages(sub_notes, video_title, ctx.language),
                temperature=0.3,
            )
            await ctx.progress.publish(
                PipelinePhase.notegen, 1.0, "Notes generated"
            )

        return StageResult(
            outputs={},
            extra={"notes": normalize_note_markdown(notes)},
        )

    @staticmethod
    async def _video_title(ctx: StageContext) -> str | None:
        """Best-effort video title from the video_meta artifact (optional).

        Only URL-path tasks have a ``video_meta`` artifact; uploads and
        missing/invalid metadata degrade to no title context.
        """
        meta = await ctx.artifacts.get(ArtifactKind.video_meta)
        if isinstance(meta, dict):
            title = meta.get("title")
            if isinstance(title, str) and title:
                return title
        return None
